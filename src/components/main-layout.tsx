...
export default function MainLayout({
  ...
  const files = [
    { name: "experiencia.html", path: "/experiencia" },
    { name: "projetos.html", path: "/projetos" },
    { name: "competencias.html", path: "/competencias" },
    { name: "galeria.html", path: "/galeria" },
  ];
              ...
              >
                ...
                {files.map((file) => (
                  <File
                    ...
                    name={file.name}
                    path={file.path}
                    active={file.name === activeItem}
                    onClick={() => {
                      if (!tabs.some((tab) => tab.name === file.name)) {
                        setTabs([...tabs, file]);
                      }
          ...
          <div className="flex overflow-x-auto overflow-y-hidden">
            {tabs.map((tab) => (
              <Tab
                ...
                icon={tab.name === "Welcome" ? "home" : "code"}
                onClose={async (_event, active) => {
                  setTabs(tabs.filter((t) => t !== tab));

                  if (active) {
                    router.push((await getTabs())[0].path);
                  }
                }}
              />
            ))}
